// Staggering Beauty — main.js (PaperScript)
// Adapted for background use: no splash, no audio, white worm on dark bg
...
// hack for 1px line (original uses id="canvas")
...
// Use document-level mouse events (canvas has pointer-events: none)
document.addEventListener('mousemove', function(e) {
  if (params.needsDrag && !mouseDown) return;
  var a = Math.atan2(e.clientY - view.size.height, e.clientX - view.size.width / 2);
...
function setPositions() {
  ...
  for (var i = 1; i < numPoints; i++) {
    ...
    var angle = Math.atan2(
      curParticle.position.y - prevParticle.position.y,
      curParticle.position.x - prevParticle.position.x
    );
...
// Start physics loop immediately (no splash needed)
